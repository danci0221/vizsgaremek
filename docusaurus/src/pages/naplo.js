...
export default function FejlesztesiNaplo() {
  ...
  useEffect(() => {
    const owner = 'danci0221';
    const repo = 'vizsgaremek';
    const url = `https://api.github.com/repos/${owner}/${repo}/commits?sha=main&per_page=200&t=${Date.now()}`;
              ...
              <div key={index} style={{
                ...
                <div style={{
                  ...
                  <div style={{ color: 'var(--ifm-color-emphasis-600)', fontSize: '0.85rem', marginBottom: '8px' }}>
                    <strong>
                      {new Date(item.commit.author.date).toLocaleDateString('hu-HU', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: '2-digit',
